...
Come visto in precedenza, l'espressione $\varphi \lor \psi$ pu\`{o} essere vista come un'abbreviazione di $\neg(\neg\varphi \wedge \neg \psi)$, ma esistono delle regole di derivazione specifiche per il connettivo $\lor$:

\subsubsection{$\lor E$ - Or Elimination}
\begin{center}
\begin{tikzpicture}
\draw (0,0) -- (4,0) node[right]{$\lor E^1$};
\draw[->][blue] (-0.5, 1.4) -- (0.5, 1.4);
\node at (-0.5,1.4)[above left][blue] {Scarico le};
\node at (-0.5,1.4)[below left][blue] {ipotesi};
\node at (2,1.2)[above] {$[\varphi]^1$};
\node at (2,1) {$\vdots$};
\node at(2, 0)[above] {$\gamma$};
\node at (3.5,1.2)[above] {$[\psi]^1$};
\node at (3.5,1) {$\vdots$};
\node at(3.5, 0)[above] {$\gamma$};
\node at (0,0)[above right] {$\varphi \lor \psi$};
\node at (2,0)[below] {$\gamma$};
\end{tikzpicture}
\end{center}
Notare che l'eliminazione dell'or prevede lo scarico delle ipotesi, e che la formula che si ottiene \`{e} diversa da quelle che compongono la formula contenente il connettivo $\lor$.
\begin{multicols}{2}
\subsubsection{$\lor I_{1}$ - Or Introduction} 
\begin{center}
\begin{tikzpicture}
\draw (0,0) -- (2,0) node[right]{$\lor I_1$};
\node at(1, 0)[above] {$\varphi$};
\node at (1,0)[below] {$\varphi \lor \psi$};
\end{tikzpicture}
\end{center}

\subsubsection{$\lor I_{2}$ - Or Introduction}
\begin{center}
\begin{tikzpicture}
\draw (0,0) -- (2,0) node[right]{$\lor I_2$};
\node at(1, 0)[above] {$\psi$};
\node at (1,0)[below] {$\varphi \lor \psi$};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}
\node at (0,1.7) [right] {$\Gamma'$};
\node at (0,1) [right] {$\mathcal{D}'$}; 
\node at (0,0) [above right] {$\varphi$};
\node at (2.75,1.7) [left] {$\Gamma''$};
\node at (2.75,1) [left] {$\mathcal{D}''$};
\node at (3,0) [above left] {$\varphi \to \psi$};
\draw (0,0) -- (3,0) node[right] {$\to E$};
\node at (1.5,0) [below] {$\psi$};

\node at (4.5,1) {$\Longrightarrow$};

\node at (7, 1.7) {$\Gamma' \vDash \varphi$};
\node at (7, 0) [above] {$\Gamma'' \vDash \varphi \to \psi$};
\end{tikzpicture}
\end{center}
